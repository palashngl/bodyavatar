#!/usr/bin/env python3
"""Train BodyAvatar v3 on NeuMan — U-Net refiner + stronger perceptual loss."""
...
def train_one_subject(
    ...
    model = BodyAvatarModelV3(image_size=args.image_size, knn_k=args.knn_k).to(device)
...
def main() -> None:
    ...
    p.add_argument("--official-smpl-dir", default="data/body/neuman_official")
    p.add_argument("--sequences", nargs="+", default=DEFAULT_SEQUENCES)
    p.add_argument("--output", default="runs/body_neuman_v3")
    ...
    official_root = Path(args.official_smpl_dir)
    ...
    for seq in args.sequences:
        ...
        npz = subject_dir / "processed.npz"
        if not npz.exists():
            print(f"Missing {subject} — run prepare_neuman.py")
            continue

        off_dir = official_root / seq
        if off_dir.exists():
            if merge_smpl_into_npz(npz, off_dir):
                print(f"  merged SMPL joints for {subject}")
        ...
        print(f"\n=== {subject} v3 ({len(train)} train / {len(val)} val) ===")
    ...
    report = {
        "dataset": "NeuMan",
        "model": "BodyAvatar v3 (U-Net refiner + multi-scale loss)",
