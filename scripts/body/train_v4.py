#!/usr/bin/env python3
"""Train BodyAvatar v4 on synthetic data (pretrain)."""
...
from osa_body.paths import default_bench_split, default_processed_dir
...
def main() -> None:
    ...
    p.add_argument("--output", default="runs/body_v4_synthetic")
    ...
    p.add_argument("--temporal-weight", type=float, default=0.06)
    args = p.parse_args()

    data_root = Path(args.data) if args.data else default_processed_dir()
    split_file = Path(args.split_file) if args.split_file else default_bench_split()
    set_seed(42)
    device = torch.device(f"cuda:{args.gpu}" if torch.cuda.is_available() else "cpu")
    out_dir = Path(args.output)
    out_dir.mkdir(parents=True, exist_ok=True)

    bank = BodyTrainBank(data_root, split_file, warp="piecewise")
    bank.preload()
    if torch.cuda.is_available():
        for subj in sorted({p.parent.name for p in data_root.glob("*/processed.npz")}):
            if subj.startswith("body_"):
                bank.pin_to_gpu(subj, device)
    train_loader = DataLoader(
        BodyVideoDataset(data_root, args.clip_length, "train", split_file),
        batch_size=1,
        shuffle=True,
        num_workers=0,
    ...
    val_loader = DataLoader(
        BodyVideoDataset(data_root, args.clip_length, "val", split_file),
        batch_size=1,
        shuffle=False,
        num_workers=0,
        collate_fn=collate_fn,
    ...
    model = BodyAvatarModelV4(knn_k=args.knn_k).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            loss, _ = v4_composite_loss(pf, tf, lpips_fn, model_out["confidence"])
        ...
        if metrics["combined"] >= best:
            ...
            torch.save(
                {"model": model.state_dict(), "metrics": metrics, "model_type": "body_v4", "args": vars(args)},
                out_dir / "best.pt",
            )
        print(f"ep {epoch+1}/{args.epochs} val psnr={metrics['psnr']:.2f} ssim={metrics['ssim']:.3f} lpips={metrics['lpips']:.3f}")

    (out_dir / "train_summary.json").write_text(json.dumps({"best_combined": best}, indent=2))
